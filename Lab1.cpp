...
*/
void johnson_algorithm(int Tablica[][2], vector<int>& task_sequence,int n){
	queue <int> first_tasks;		//tasks on 1 machine
	stack <int> second_tasks;		//task on second machine
//	vector <int> task_sequence;		//first+second tasks 
	bool tasks_list[5];				// tasks to do
	int prev_min=0;					//prev task min
	int which_task,which_machine=0;
	fill_n(tasks_list,n,1);	        //fill table with n elem. Set on 1		
	for(int iter=0;iter<n;iter++) 
	{
		int min=1000;		//1000 some great value;
		for(int i=0; i<n;i++)
		{
	...
	cout<<"Optymalna Sekwencja wykonania zadan(Algorytm Johnsona) to"<<endl;
 	for( int i = 0;i<task_sequence.size(); i++ )
       cout<<" "<< task_sequence[i];
    cout<<endl;
}
...
int main()
{
	...
	int data2[5][2]={{9,2},{5,7},{8,7},{4,5,},{3,1}};
	...
	print_matrix();
	find_min_cmax();
	johnson_algorithm(data2,task_sequence,5);
	//reduce_col(data3);
